...
@router.get("/stats")
async def get_dashboard_stats(current_user: Any = Depends(deps.get_current_active_user)):
    """
...
async def get_telemetry(current_user: Any = Depends(deps.get_current_active_user)):
    """
...
async def get_progression(current_user: Any = Depends(deps.get_current_active_user)):
    """
